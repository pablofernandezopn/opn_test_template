...
class Environment {
  Environment._internal(BuildVariant buildVariant) {
    ...
    switch (_buildVariant) {
      ...
      // Obtener IP desde variable de entorno o usar default
        const localIp = String.fromEnvironment(
          'LOCAL_IP',
          defaultValue: '127.0.0.1',
        );

        // Si es Android y no se especificó IP personalizada, usa 10.0.2.2 para emulador
        // En web, siempre usa la IP especificada (o 127.0.0.1 por defecto)
        final baseIp = !kIsWeb && Platform.isAndroid && localIp == '127.0.0.1'
            ? '10.0.2.2'
            : localIp;

        _supabaseUrl = 'http://$baseIp:54321';
        _supabaseKey = 'sb_publishable_ACJWlzQHlZjBrEguHvfOxg_3BJgxAaH';

        // Credenciales adicionales de S3 local
      ...
      // 🚀 Configuración de Supabase en producción
        _supabaseUrl = 'https://toraglnkbwgufwmyauvp.supabase.co';
        _supabaseKey =
        'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRvcmFnbG5rYndndWZ3bXlhdXZwIiwicm9sZSI6ImFub24iLCJpYXQiOjE2NjY3MDc0MzEsImV4cCI6MTk4MjI4MzQzMX0._CHEagqw4yN0RCUd5grlZ5I5dO6rRjDMcM_eskznrGM';
  ...
  void logConnectionInfo() {
    if (kDebugMode) {
      ...
      if (isLocal) {
        ...
        print('      - Web: Usa 127.0.0.1 (automático)');
